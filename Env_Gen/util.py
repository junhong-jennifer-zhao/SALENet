...
def save_current_images(visuals, epoch, step, vis_path):
    ## convert tensors to numpy arrays
    visuals = convert_visuals_to_numpy(visuals)

    # if self.use_html:  # save images to a html file
    for label, image_numpy in visuals.items():
        img_path = os.path.join(vis_path, 'epoch%.3d_iter%.3d_%s.png' % (epoch, step, label))
        if label == 'input':
...
def save_test_images(visuals, nm, result_dir):
    ## convert tensors to numpy arrays
    visuals = convert_visuals_to_numpy(visuals)
    
    save_dir = os.path.join(result_dir, nm)
    ...
    for label, image_numpy in visuals.items():
        ...
        if label == 'fake_image':
            im = image_numpy
            im_path = save_dir + '/' + nm + '_' + label + '.jpg'
            # print("im_path:",im_path)
            tonemapping(im, im_path)

            hdr_path = save_dir + '/' + nm + '_' + label + '.exr'
        ...
        if label == 'input':
            im_path = save_dir + '/' + nm + '_' + 'SG' + '.jpg'
            print("im_path:",im_path)
            SG = image_numpy[:,:,:3] * 255.0
            tonemapping(SG, im_path, percentile=99, max_mapping=0.99)

            if image_numpy.shape[2] >3:
                # im_path = save_dir + nm + '_' + 'Depth' + '.jpg'
                # Depth = image_numpy[:,:,3:6] * 255.0
                # tonemapping(Depth, im_path, percentile=99, max_mapping=0.99)
                im_path = save_dir + '/' + nm + '_' + 'SH' + '.jpg'
                SH = image_numpy[:,:,3:6] * 255.0
                tonemapping(SH, im_path, percentile=99, max_mapping=0.99)
            
            if image_numpy.shape[2] ==9:
                im_path = save_dir + '/' + nm + '_' + 'SH' + '.jpg'
                SH = image_numpy[:,:,6:9] * 255.0
                tonemapping(SH, im_path, percentile=99, max_mapping=0.99)
        ...
        if label == 'im':
            ...
            im_path = save_dir + '/' + nm + '_' + 'crop' + '.jpg'
            hdr = image_numpy * 255.0
            hdr = Image.fromarray(hdr.astype('uint8'))
            hdr.save(im_path)
