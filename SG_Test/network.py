...
import timm
from Conformer.models import Conformer_base_patch16
...
def conformer(pretrained=True, **kwargs): 
    predModelPath='./Conformer/model/Conformer_base_patch16.pth'
    nb_classes = 0
    global_pool=False
    model = Conformer_base_patch16(is_multi_proj=True)
    if pretrained==True:
        print("Load pre-trained checkpoint from: %s" % predModelPath)
...
if __name__ == '__main__':
    ...
    out = net(x)
    temp = out['distribution']
    util.print_model_parm_nums(net)
    print(temp.shape)
    # load_model(net,"dla60x.pth")
    # # net.load_state_dict(torch.load("dla60x.pth"))
    # # print(out.shape)
    # # torch.save(net.state_dict(), 'latest_net.pth')
    # oldpth = net.state_dict()
    # new_dict = OrderedDict()
    # for key, v in oldpth.items():
    #     new_dict['module.'+key] = oldpth[key]
    #
    # print(new_dict.keys())
    # torch.save(new_dict, 'latest_net.pth')